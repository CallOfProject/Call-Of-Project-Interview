    ...
    acceptTestInterview(id: string, status: boolean) {
        return this.http.post<any>(acceptOrRejectTestInterviewRequest(id, status), null, {
            ...
            .pipe(
                map((response: any) => {
                    if (response.status_code === 2000) {
                        const resultDTO = new InterviewResultDTO()
                        resultDTO.email = response.object.email
                        resultDTO.message = response.object.message
                        resultDTO.ownerId = response.object.ownerId
                        resultDTO.projectName = response.object.projectName
                        resultDTO.userId = response.object.userId
                        return resultDTO
                    }
                    return null;
                }),
