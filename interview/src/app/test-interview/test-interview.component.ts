...
//http://localhost:4201/test-interview?interview_id=229a9bfb-55ee-4105-81d8-15aeeef0fba8&user_id=0cb9844a-a538-4802-9665-fd4cd2a6536f
    ngOnInit(): void {
        ...
        const interview_id = localStorage.getItem("test_interview_id");
        if (interview_id !== this.interviewId) {
            localStorage.clear()

        }
        localStorage.setItem("test_interview_id", this.interviewId);
        ...
        this.submitService.getTestInterviewInformation(this.interviewId).subscribe((response: any) => {
            if (response.status_code === 2000) {
                ...
                if (localStorage.getItem(KEY) === DEFAULT.toString()) {
                    localStorage.setItem(KEY, `${response.object.duration_time * 60}`);
                    this.config = {...this.config, leftTime: response.object.duration_time * 60};
                }
